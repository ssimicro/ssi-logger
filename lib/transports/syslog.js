...
function SysLog(name, options) {
    ...
    this.facility = nodeSyslog[options.facility] || nodeSyslog.LOG_MESSAGES;
...
module.exports = function syslogTransport(facility, mask) {

    var self = this;

    self.syslog = new SysLog(process.title, { facility: facility, mask: mask });
