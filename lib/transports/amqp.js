...
class AmqpTransport extends Transport{
    constructor(options) {
        ...
        this.producer = new AmqpProducer(options);
    }

    // Establish the AMQP connection. This replaces the former synchronous
    // (deasync) connect that ran in the constructor; log.open() awaits this.
    // Messages logged before the connection is ready are buffered by the
    // producer's queue and flushed once connected. On connection failure the
    // configured onConnectError handler is invoked (default: process.exit(1));
    // if that handler throws, open() rejects with the thrown error.
    async open() {
    ...
    log(log_event) {
        ...
        this.producer.publish(amq_msg);
    }

    end(optDone) {
        this.producer.end(optDone);
    }
