...
function logEventToJsonPayload(log_event, headers, amq_msg) {
    ...
    amq_msg.payload.log_details = convertArraysToObjects(log_event.data.slice(args_used));
    ...
    connect(callback) {
        ...
        if (this.chan) {
            ...
            try {
                ...
                // Avoid leaking a connection handle.
                this.chan.close((err) => {
                    this.chan = null;
                    this.conn.close((err) => {
                        this.conn = null;
...
module.exports.convertArraysToObjects = convertArraysToObjects;
