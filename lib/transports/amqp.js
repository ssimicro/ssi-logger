...
module.exports = function amqpTransport(options, optDone) {
        ...
        return _.reduce(obj, (acc, value, key) => {
            if (_.isString(value)) {
                ...
                acc[key] = value.reduce((acc, value, key) => {
                    acc[key] = _.isObject(value) ? arraysToObjects(value) : value;
                    return acc;
                }, {});
            } else if (_.isObject(value)) {
                acc[key] = arraysToObjects(value);
            } else {
                acc[key] = value;
            }
            return acc;
    ...
    return function amqpLogEventClosure(log_event) {
        ...
        const amq_msg = {
            payload: options.format !== 'json' ? log_event.message : {
                log_message: null,
                log_details: [],
            },
            publishOptions: {
        ...
        if (options.format === 'json') {
            let args_used = 0;
            if (semver.satisfies(log_event.version, '^1.0.0')) {
            ...
            amq_msg.payload.log_details = arraysToObjects(log_event.data.slice(args_used));

            amq_msg.payload.log_metadata = {
                ver: "1.0.0",
                host: log_event.host,
                process: proc_name,
                level: log_event.level,
