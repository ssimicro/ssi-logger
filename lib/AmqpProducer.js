...
class AmqpProducer extends EventEmitter {
    ...
    bail(err) {
        this.end();
        this.save_stack(err, "AMQP TRANSPORT error event");
    ...
    closed(err) {
        ...
        this.save_stack(err, "AMQP TRANSPORT connection closed event");
        this.end();
        if (err) {
    ...
    closed_channel(err) {
        ...
        if (this.options.level === 'DEBUG') {
            console.error('AMQP TRANSPORT channel closed event', err || '');
        }
        if (err) {
            this.save_stack(err, "AMQP TRANSPORT channel closed event");
        }
    }
    ...
    reconnect(err, reconnectError) {
        if (this.options.level === 'DEBUG') {
            console.error('AMQP TRANSPORT reconnecting ...');
        }
        if (err) {
