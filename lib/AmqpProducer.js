...
class AmqpProducer extends EventEmitter {
    ...
    closed(err) {
        ...
        this.close();
        if (err && _.get(err, 'code', -1) !== 320 /* Forced-Closed */) {
        ...
        amqp.connect(this.options.url, this.options.socketOptions, _.once((err, conn) => {
            ...
            this.conn.createConfirmChannel((err, chan) => {
                ...
                this.chan.assertExchange(this.options.exchangeName, 'topic', this.options.exchangeOptions, (err, ok) => {
                    ...
                    if (this.options.traceLevel > 0) {
                        this.save_stack(null, "AMQP LOG TRANSPORT ready");
                    }
        ...
        if (err) {
            if (this.reconnectStopAfter === null) {
                ...
                if (this.options.traceLevel > 1) {
                    this.save_stack(null, 'AMQP LOG TRANSPORT reconnectStopAfter %s', this.reconnectStopAfter.toISOString());
                }
            }
            if (moment().isBefore(this.reconnectStopAfter)) {
                if (!this.reconnectTimer) {
                    this.reconnectCount++;
                    if (this.options.traceLevel > 1) {
                       this.save_stack(err, 'AMQP LOG TRANSPORT reconnecting ...');
                    }
                    this.reconnectTimer = setTimeout(() => {
                        this.connect((err) => {
                            this.reconnectTimer = null;
                            if (err) {
                                this.save_stack(err, 'AMQP LOG TRANSPORT reconnect error '+this.reconnectCount);
                                reconnectError(err);
                            }
                        });
                ...
                this.save_stack(null, 'AMQP LOG TRANSPORT no connection, re-emitting error');
                this.emit('error', err);
            }
        }
    }
