...
class AmqpProducer extends EventEmitter {
    ...
    save_stack(err, message) {
        try {
            ...
            console.error("failed to append error to stack file", {stack_file: this.stack_file, err: e});
    ...
    connect(callback) {
        ...
        amqp.connect(this.options.url, this.options.socketOptions, _.once((err, conn) => {
            ...
            this.conn.createConfirmChannel((err, chan) => {
                if (err) {
                    err.options = this.options;
                    this.save_stack(err, 'AMQP LOG TRANSPORT channel error');
                    this.conn.close();
                    return callback(err);
                }
                this.chan = chan;
                ...
                this.chan.assertExchange(this.options.exchangeName, 'topic', this.options.exchangeOptions, (err, ok) => {
                    if (err) {
                        err.options = this.options;
                        this.save_stack(err, 'AMQP LOG TRANSPORT assert exchange error');
                        this.close();
                        return callback(err);
                    }
    ...
    reconnect() {
        ...
        if (moment().isBefore(this.reconnectStopAfter)) {
            ...
            this.save_stack(err, "AMQP LOG TRANSPORT connection gone");
